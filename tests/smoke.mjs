...
crossPrelude.next = "";
const crossStart = createFrame("dialogue");
...
compactCrossBranch.frames = [crossPrelude, crossStart, crossEntry, crossEnd];
compactCrossBranch.startFrame = crossPrelude.id;
const compactCrossBranchGraph = graph._buildGraph(compactCrossBranch);
assert.equal(compactCrossBranchGraph.nodes.some(node => node.id === crossStart.id && !node.isVirtual), true, "Compact graph must keep the non-start source side of a cross-branch transition visible");
...
assert.equal(compactCrossBranchGraph.edges.some(edge => edge.sourceId === crossPrelude.id && edge.targetId === crossStart.id && !edge.isBroken), true, "Compact graph must retain the linear path into a non-start cross-branch source");
