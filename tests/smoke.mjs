...
const crossPrelude = compactCrossBranch.frames[0];
crossPrelude.id = "cross-prelude";
crossPrelude.branchId = compactBranchA.id;
crossPrelude.isFinal = false;
crossPrelude.next = "cross-a1";
const crossA1 = createFrame("dialogue");
crossA1.id = "cross-a1";
crossA1.branchId = compactBranchA.id;
crossA1.isFinal = false;
crossA1.next = "cross-b1";
const crossB1 = createFrame("dialogue");
crossB1.id = "cross-b1";
crossB1.branchId = compactBranchB.id;
crossB1.isFinal = false;
crossB1.next = "cross-a2";
const crossA2 = createFrame("dialogue");
crossA2.id = "cross-a2";
crossA2.branchId = compactBranchA.id;
crossA2.isFinal = false;
crossA2.next = "cross-end";
const crossEnd = createFrame("dialogue");
crossEnd.id = "cross-end";
crossEnd.branchId = compactBranchB.id;
crossEnd.isFinal = true;
compactCrossBranch.frames = [crossPrelude, crossA1, crossB1, crossA2, crossEnd];
compactCrossBranch.startFrame = crossPrelude.id;
const compactCrossBranchGraph = graph._buildGraph(compactCrossBranch);
assert.equal(compactCrossBranchGraph.visibleFrameCount, 2, "Compact graph must collapse linear dialogue frames even when the path crosses branches repeatedly");
assert.equal(compactCrossBranchGraph.nodes.some(node => node.id === crossA1.id), false, "A linear source before a cross-branch hand-off must remain collapsible");
assert.equal(compactCrossBranchGraph.nodes.some(node => node.id === crossB1.id), false, "A linear cross-branch destination must remain collapsible");
assert.equal(compactCrossBranchGraph.nodes.some(node => node.id === crossA2.id), false, "Returning to another editor branch must not force a linear frame visible");
assert.equal(compactCrossBranchGraph.nodes.some(node => node.isBroken), false, "Collapsed cross-branch chains must not produce broken-link nodes");
assert.equal(compactCrossBranchGraph.edges.some(edge => edge.sourceId === crossPrelude.id && edge.targetId === crossEnd.id && !edge.isBroken), true, "Compact graph must bridge a multi-branch linear chain directly to the next visible anchor");
